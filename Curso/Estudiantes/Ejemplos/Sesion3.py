...
def test_sesion_val_nombre(set_up_val_nom)-> None:
    ...
    F=Funciones_Globales(page)
    #F.Validar_titulo_pagina("Swag Labs")       
    texto=F.Validar_Texto("//h3[@data-test='error'][contains(.,'Epic sadface: Username and password do not match any user in this service')]",1)
    print(texto)
...
#Para el Error de password
def test_sesion_val_password(set_up_val_password)-> None:
    page=set_up_val_password
    ...
    print(texto)
    texto_esperar="Username and password do not match any user in this service"
    assert texto_esperar in str(texto), "Password Incorrecto"
